...
func NewAdminUserInteractor(authConfig domain.AuthConfig, dataStore domain.DataStore) *AdminUserInteractor {
...
func (interactor *AdminUserInteractor) Login(ctx context.Context, username, password string) (token string, expAt time.Time, err error) {
	...
	user, err := interactor.DataStore.GetAdminUserRepository().FindByUsername(ctx, username)
	if err != nil {
		return
	}
...
func (interactor *AdminUserInteractor) Authenticate(ctx context.Context, token string) (user *domain.AdminUser, err error) {
	...
	user, err = interactor.DataStore.GetAdminUserRepository().Find(ctx, claims.ID)
	if err != nil {
