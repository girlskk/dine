...
* post 请求
 */
func DoPostReq(ctx context.Context, url string, params map[string]interface{}, msc *MerchSysConfig) (*http.Response, error) {
	// 构建参数字符串
	...
	request, _ := http.NewRequestWithContext(ctx, "POST", url, strings.NewReader(respDataText))
...
func PostRequest(ctx context.Context, reqUrl string, reqParam map[string]interface{}, msc *MerchSysConfig, needVerfySign ...bool) (map[string]interface{}, error) {
	var resp *http.Response
	var err error

	resp, err = DoPostReq(ctx, reqUrl, reqParam, msc)
