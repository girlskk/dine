...
type OrderItemQuery struct {
	...
	withSetMealDetails *OrderItemSetMealDetailQuery
...
func (oiq *OrderItemQuery) QuerySetMealDetails() *OrderItemSetMealDetailQuery {
	query := (&OrderItemSetMealDetailClient{config: oiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oiq *OrderItemQuery) Clone() *OrderItemQuery {
	...
	return &OrderItemQuery{
		...
		withSetMealDetails: oiq.withSetMealDetails.Clone(),
...
func (oiq *OrderItemQuery) WithSetMealDetails(opts ...func(*OrderItemSetMealDetailQuery)) *OrderItemQuery {
	query := (&OrderItemSetMealDetailClient{config: oiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oiq.withSetMealDetails = query
	return oiq
}
...
func (oiq *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	...
	if query := oiq.withSetMealDetails; query != nil {
		if err := oiq.loadSetMealDetails(ctx, query, nodes,
			func(n *OrderItem) { n.Edges.SetMealDetails = []*OrderItemSetMealDetail{} },
			func(n *OrderItem, e *OrderItemSetMealDetail) {
				n.Edges.SetMealDetails = append(n.Edges.SetMealDetails, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (oiq *OrderItemQuery) loadSetMealDetails(ctx context.Context, query *OrderItemSetMealDetailQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *OrderItemSetMealDetail)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orderitemsetmealdetail.FieldOrderItemID)
	}
	query.Where(predicate.OrderItemSetMealDetail(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(orderitem.SetMealDetailsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
