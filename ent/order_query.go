...
type OrderQuery struct {
	...
	withLogs             *OrderLogQuery
	withCurrentDinetable *DineTableQuery
	withDinetable        *DineTableQuery
...
func (oq *OrderQuery) QueryLogs() *OrderLogQuery {
	query := (&OrderLogClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryCurrentDinetable chains the current query on the "current_dinetable" edge.
func (oq *OrderQuery) QueryCurrentDinetable() *DineTableQuery {
	query := (&DineTableClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryDinetable chains the current query on the "dinetable" edge.
func (oq *OrderQuery) QueryDinetable() *DineTableQuery {
	query := (&DineTableClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withLogs:             oq.withLogs.Clone(),
		withCurrentDinetable: oq.withCurrentDinetable.Clone(),
		withDinetable:        oq.withDinetable.Clone(),
...
func (oq *OrderQuery) WithLogs(opts ...func(*OrderLogQuery)) *OrderQuery {
	query := (&OrderLogClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithCurrentDinetable tells the query-builder to eager-load the nodes that are connected to
// the "current_dinetable" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithCurrentDinetable(opts ...func(*DineTableQuery)) *OrderQuery {
	query := (&DineTableClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withCurrentDinetable = query
	return oq
}

// WithDinetable tells the query-builder to eager-load the nodes that are connected to
// the "dinetable" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithDinetable(opts ...func(*DineTableQuery)) *OrderQuery {
	query := (&DineTableClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withDinetable = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	var (
		...
		loadedTypes = [4]bool{
			oq.withItems != nil,
			oq.withLogs != nil,
			oq.withCurrentDinetable != nil,
			oq.withDinetable != nil,
		}
	)
	...
	if query := oq.withLogs; query != nil {
		if err := oq.loadLogs(ctx, query, nodes,
			func(n *Order) { n.Edges.Logs = []*OrderLog{} },
			func(n *Order, e *OrderLog) { n.Edges.Logs = append(n.Edges.Logs, e) }); err != nil {
			return nil, err
		}
	}
	if query := oq.withCurrentDinetable; query != nil {
		if err := oq.loadCurrentDinetable(ctx, query, nodes, nil,
			func(n *Order, e *DineTable) { n.Edges.CurrentDinetable = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withDinetable; query != nil {
		if err := oq.loadDinetable(ctx, query, nodes, nil,
			func(n *Order, e *DineTable) { n.Edges.Dinetable = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadLogs(ctx context.Context, query *OrderLogQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orderlog.FieldOrderID)
	}
	query.Where(predicate.OrderLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.LogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (oq *OrderQuery) loadCurrentDinetable(ctx context.Context, query *DineTableQuery, nodes []*Order, init func(*Order), assign func(*Order, *DineTable)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(dinetable.FieldOrderID)
	}
	query.Where(predicate.DineTable(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.CurrentDinetableColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (oq *OrderQuery) loadDinetable(ctx context.Context, query *DineTableQuery, nodes []*Order, init func(*Order), assign func(*Order, *DineTable)) error {
	...
	query.Where(dinetable.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
