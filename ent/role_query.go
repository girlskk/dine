...
func (rq *RoleQuery) QueryMerchant() *MerchantQuery {
...
func (rq *RoleQuery) QueryStore() *StoreQuery {
...
func (rq *RoleQuery) WithMerchant(opts ...func(*MerchantQuery)) *RoleQuery {
	query := (&MerchantClient{config: rq.config}).Query()
...
func (rq *RoleQuery) WithStore(opts ...func(*StoreQuery)) *RoleQuery {
	query := (&StoreClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withMerchant; query != nil {
		if err := rq.loadMerchant(ctx, query, nodes, nil,
			func(n *Role, e *Merchant) { n.Edges.Merchant = e }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoleQuery) loadMerchant(ctx context.Context, query *MerchantQuery, nodes []*Role, init func(*Role), assign func(*Role, *Merchant)) error {
...
func (rq *RoleQuery) loadStore(ctx context.Context, query *StoreQuery, nodes []*Role, init func(*Role), assign func(*Role, *Store)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Role)
	for i := range nodes {
		fk := nodes[i].StoreID
...
func (rq *RoleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withMerchant != nil {
			_spec.Node.AddColumnOnce(role.FieldMerchantID)
		}
		if rq.withStore != nil {
			_spec.Node.AddColumnOnce(role.FieldStoreID)
		}
	}
