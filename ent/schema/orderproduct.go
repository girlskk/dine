...
func (OrderProduct) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int("gift_qty").Default(0).Comment("赠送数量"),
		...
		field.UUID("refunded_by", uuid.UUID{}).Optional().Comment("退菜操作人"),
		...
		field.Other("price", decimal.Decimal{}).
			...
			Nillable().
			Comment("商品价格"),
		field.JSON("groups", domain.SetMealGroups{}).Optional().Comment("套餐组信息（包含套餐组名称、选择类型、详情列表等）"),

		// ========== 规格信息 ==========
		field.JSON("spec_relations", domain.ProductSpecRelations{}).Optional().Comment("商品规格关联信息（规格名称、价格、库存等）"),

		// ========== 口味做法信息 ==========
		field.JSON("attr_relations", domain.ProductAttrRelations{}).Optional().Comment("商品口味做法关联信息（口味名称、加价等）"),
	}
}
