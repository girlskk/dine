...
func (cq *CountryQuery) QueryDistricts() *DistrictQuery {
...
func (cq *CountryQuery) QueryMerchants() *MerchantQuery {
	query := (&MerchantClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CountryQuery) QueryStores() *StoreQuery {
...
func (cq *CountryQuery) Clone() *CountryQuery {
	...
	return &CountryQuery{
		...
		withMerchants: cq.withMerchants.Clone(),
		withStores:    cq.withStores.Clone(),
...
func (cq *CountryQuery) WithDistricts(opts ...func(*DistrictQuery)) *CountryQuery {
	query := (&DistrictClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CountryQuery) WithMerchants(opts ...func(*MerchantQuery)) *CountryQuery {
	query := (&MerchantClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withMerchants = query
	return cq
}
...
func (cq *CountryQuery) WithStores(opts ...func(*StoreQuery)) *CountryQuery {
	query := (&StoreClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CountryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Country, error) {
	...
	if query := cq.withDistricts; query != nil {
		if err := cq.loadDistricts(ctx, query, nodes,
			func(n *Country) { n.Edges.Districts = []*District{} },
			func(n *Country, e *District) { n.Edges.Districts = append(n.Edges.Districts, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withMerchants; query != nil {
		if err := cq.loadMerchants(ctx, query, nodes,
			func(n *Country) { n.Edges.Merchants = []*Merchant{} },
			func(n *Country, e *Merchant) { n.Edges.Merchants = append(n.Edges.Merchants, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withStores; query != nil {
		if err := cq.loadStores(ctx, query, nodes,
			func(n *Country) { n.Edges.Stores = []*Store{} },
			func(n *Country, e *Store) { n.Edges.Stores = append(n.Edges.Stores, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CountryQuery) loadDistricts(ctx context.Context, query *DistrictQuery, nodes []*Country, init func(*Country), assign func(*Country, *District)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Country)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(district.FieldCountryID)
	}
	query.Where(predicate.District(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(country.DistrictsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CountryQuery) loadMerchants(ctx context.Context, query *MerchantQuery, nodes []*Country, init func(*Country), assign func(*Country, *Merchant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Country)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(merchant.FieldCountryID)
	}
	query.Where(predicate.Merchant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(country.MerchantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CountryQuery) loadStores(ctx context.Context, query *StoreQuery, nodes []*Country, init func(*Country), assign func(*Country, *Store)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Country)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(store.FieldCountryID)
	}
	query.Where(predicate.Store(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(country.StoresColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
