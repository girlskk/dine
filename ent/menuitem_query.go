...
func (miq *MenuItemQuery) QueryProduct() *ProductQuery {
	query := (&ProductClient{config: miq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := miq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (miq *MenuItemQuery) Clone() *MenuItemQuery {
	...
	return &MenuItemQuery{
		...
		withProduct: miq.withProduct.Clone(),
		// clone intermediate query.
		sql:       miq.sql.Clone(),
		path:      miq.path,
		modifiers: append([]func(*sql.Selector){}, miq.modifiers...),
	}
}
...
func (miq *MenuItemQuery) WithProduct(opts ...func(*ProductQuery)) *MenuItemQuery {
	query := (&ProductClient{config: miq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	miq.withProduct = query
	return miq
}
...
func (miq *MenuItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*MenuItem, error) {
	...
	if query := miq.withProduct; query != nil {
		if err := miq.loadProduct(ctx, query, nodes, nil,
			func(n *MenuItem, e *Product) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (miq *MenuItemQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*MenuItem, init func(*MenuItem), assign func(*MenuItem, *Product)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*MenuItem)
	for i := range nodes {
...
func (miq *MenuItemQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := miq.ctx.Fields; len(fields) > 0 {
		...
		if miq.withProduct != nil {
			_spec.Node.AddColumnOnce(menuitem.FieldProductID)
		}
	}
...
func (miq *MenuItemQuery) ForUpdate(opts ...sql.LockOption) *MenuItemQuery {
...
func (miq *MenuItemQuery) ForShare(opts ...sql.LockOption) *MenuItemQuery {
	if miq.driver.Dialect() == dialect.Postgres {
		miq.Unique(false)
	}
	miq.modifiers = append(miq.modifiers, func(s *sql.Selector) {
...
func (miq *MenuItemQuery) Modify(modifiers ...func(s *sql.Selector)) *MenuItemSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (mis *MenuItemSelect) Modify(modifiers ...func(s *sql.Selector)) *MenuItemSelect {
