...
func (dq *DeviceQuery) QueryMerchant() *MerchantQuery {
...
func (dq *DeviceQuery) QueryStore() *StoreQuery {
...
func (dq *DeviceQuery) QueryStall() *StallQuery {
	query := (&StallClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DeviceQuery) WithMerchant(opts ...func(*MerchantQuery)) *DeviceQuery {
	query := (&MerchantClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withMerchant = query
	return dq
}
...
func (dq *DeviceQuery) WithStore(opts ...func(*StoreQuery)) *DeviceQuery {
	query := (&StoreClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DeviceQuery) WithStall(opts ...func(*StallQuery)) *DeviceQuery {
	query := (&StallClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Device, error) {
	...
	if query := dq.withMerchant; query != nil {
		if err := dq.loadMerchant(ctx, query, nodes, nil,
			func(n *Device, e *Merchant) { n.Edges.Merchant = e }); err != nil {
			return nil, err
		}
	}
	if query := dq.withStore; query != nil {
		if err := dq.loadStore(ctx, query, nodes, nil,
			func(n *Device, e *Store) { n.Edges.Store = e }); err != nil {
			return nil, err
		}
	}
	if query := dq.withStall; query != nil {
		if err := dq.loadStall(ctx, query, nodes, nil,
			func(n *Device, e *Stall) { n.Edges.Stall = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DeviceQuery) loadMerchant(ctx context.Context, query *MerchantQuery, nodes []*Device, init func(*Device), assign func(*Device, *Merchant)) error {
...
func (dq *DeviceQuery) loadStore(ctx context.Context, query *StoreQuery, nodes []*Device, init func(*Device), assign func(*Device, *Store)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Device)
	for i := range nodes {
		fk := nodes[i].StoreID
...
func (dq *DeviceQuery) loadStall(ctx context.Context, query *StallQuery, nodes []*Device, init func(*Device), assign func(*Device, *Stall)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Device)
	for i := range nodes {
		fk := nodes[i].StallID
...
func (dq *DeviceQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := dq.ctx.Fields; len(fields) > 0 {
		...
		if dq.withMerchant != nil {
			_spec.Node.AddColumnOnce(device.FieldMerchantID)
		}
		if dq.withStore != nil {
			_spec.Node.AddColumnOnce(device.FieldStoreID)
		}
		if dq.withStall != nil {
			_spec.Node.AddColumnOnce(device.FieldStallID)
		}
	}
