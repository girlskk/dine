...
func (cq *CityQuery) QueryMerchants() *MerchantQuery {
...
func (cq *CityQuery) QueryStores() *StoreQuery {
...
func (cq *CityQuery) WithMerchants(opts ...func(*MerchantQuery)) *CityQuery {
	query := (&MerchantClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CityQuery) WithStores(opts ...func(*StoreQuery)) *CityQuery {
	query := (&StoreClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*City, error) {
	...
	if query := cq.withMerchants; query != nil {
		if err := cq.loadMerchants(ctx, query, nodes,
			func(n *City) { n.Edges.Merchants = []*Merchant{} },
			func(n *City, e *Merchant) { n.Edges.Merchants = append(n.Edges.Merchants, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withStores; query != nil {
		if err := cq.loadStores(ctx, query, nodes,
			func(n *City) { n.Edges.Stores = []*Store{} },
			func(n *City, e *Store) { n.Edges.Stores = append(n.Edges.Stores, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CityQuery) loadMerchants(ctx context.Context, query *MerchantQuery, nodes []*City, init func(*City), assign func(*City, *Merchant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*City)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(merchant.FieldCityID)
	}
	query.Where(predicate.Merchant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(city.MerchantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CityQuery) loadStores(ctx context.Context, query *StoreQuery, nodes []*City, init func(*City), assign func(*City, *Store)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*City)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(store.FieldCityID)
	}
	query.Where(predicate.Store(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(city.StoresColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CityQuery) ForUpdate(opts ...sql.LockOption) *CityQuery {
...
func (cq *CityQuery) ForShare(opts ...sql.LockOption) *CityQuery {
	if cq.driver.Dialect() == dialect.Postgres {
		cq.Unique(false)
	}
	cq.modifiers = append(cq.modifiers, func(s *sql.Selector) {
