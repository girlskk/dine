...
func (pq *ProductQuery) QueryUnit() *ProductUnitQuery {
	query := (&ProductUnitClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) QueryProductSpecs() *ProductSpecRelationQuery {
	query := (&ProductSpecRelationClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) QueryProductAttrs() *ProductAttrRelationQuery {
	query := (&ProductAttrRelationClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) WithUnit(opts ...func(*ProductUnitQuery)) *ProductQuery {
	query := (&ProductUnitClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) WithProductSpecs(opts ...func(*ProductSpecRelationQuery)) *ProductQuery {
	query := (&ProductSpecRelationClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) WithProductAttrs(opts ...func(*ProductAttrRelationQuery)) *ProductQuery {
	query := (&ProductAttrRelationClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withTags; query != nil {
		if err := pq.loadTags(ctx, query, nodes,
			func(n *Product) { n.Edges.Tags = []*ProductTag{} },
			func(n *Product, e *ProductTag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withProductSpecs; query != nil {
		if err := pq.loadProductSpecs(ctx, query, nodes,
			func(n *Product) { n.Edges.ProductSpecs = []*ProductSpecRelation{} },
			func(n *Product, e *ProductSpecRelation) { n.Edges.ProductSpecs = append(n.Edges.ProductSpecs, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withProductAttrs; query != nil {
...
func (pq *ProductQuery) loadUnit(ctx context.Context, query *ProductUnitQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductUnit)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Product)
	for i := range nodes {
		fk := nodes[i].UnitID
	...
	query.Where(productunit.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
...
func (pq *ProductQuery) loadTags(ctx context.Context, query *ProductTagQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductTag)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(producttag.FieldID), joinT.C(product.TagsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*ProductTag](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
...
func (pq *ProductQuery) loadProductSpecs(ctx context.Context, query *ProductSpecRelationQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductSpecRelation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(productspecrelation.FieldProductID)
	}
...
func (pq *ProductQuery) loadProductAttrs(ctx context.Context, query *ProductAttrRelationQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductAttrRelation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(productattrrelation.FieldProductID)
	}
