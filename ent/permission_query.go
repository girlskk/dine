...
func (pq *PermissionQuery) QueryMenu() *RouterMenuQuery {
	query := (&RouterMenuClient{config: pq.config}).Query()
...
func (pq *PermissionQuery) WithMenu(opts ...func(*RouterMenuQuery)) *PermissionQuery {
	query := (&RouterMenuClient{config: pq.config}).Query()
...
func (pq *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := pq.withMenu; query != nil {
		if err := pq.loadMenu(ctx, query, nodes, nil,
			func(n *Permission, e *RouterMenu) { n.Edges.Menu = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PermissionQuery) loadMenu(ctx context.Context, query *RouterMenuQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *RouterMenu)) error {
	...
	query.Where(routermenu.IDIn(ids...))
...
func (pq *PermissionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withMenu != nil {
			_spec.Node.AddColumnOnce(permission.FieldMenuID)
		}
	}
