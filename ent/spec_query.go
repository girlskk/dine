...
func (sq *SpecQuery) QueryProductSpecs() *ProductSpecQuery {
	query := (&ProductSpecClient{config: sq.config}).Query()
...
func (sq *SpecQuery) Clone() *SpecQuery {
	...
	return &SpecQuery{
		...
		withProductSpecs: sq.withProductSpecs.Clone(),
...
func (sq *SpecQuery) WithProductSpecs(opts ...func(*ProductSpecQuery)) *SpecQuery {
	query := (&ProductSpecClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withProductSpecs = query
	return sq
}
...
func (sq *SpecQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Spec, error) {
	...
	if query := sq.withProductSpecs; query != nil {
		if err := sq.loadProductSpecs(ctx, query, nodes,
			func(n *Spec) { n.Edges.ProductSpecs = []*ProductSpec{} },
			func(n *Spec, e *ProductSpec) { n.Edges.ProductSpecs = append(n.Edges.ProductSpecs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SpecQuery) loadProductSpecs(ctx context.Context, query *ProductSpecQuery, nodes []*Spec, init func(*Spec), assign func(*Spec, *ProductSpec)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Spec)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(productspec.FieldSpecID)
	}
	query.Where(predicate.ProductSpec(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(spec.ProductSpecsColumn), fks...))
	}))
