...
func (dq *DepartmentQuery) QueryMerchant() *MerchantQuery {
...
func (dq *DepartmentQuery) QueryStore() *StoreQuery {
...
func (dq *DepartmentQuery) WithMerchant(opts ...func(*MerchantQuery)) *DepartmentQuery {
	query := (&MerchantClient{config: dq.config}).Query()
...
func (dq *DepartmentQuery) WithStore(opts ...func(*StoreQuery)) *DepartmentQuery {
	query := (&StoreClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withMerchant; query != nil {
		if err := dq.loadMerchant(ctx, query, nodes, nil,
			func(n *Department, e *Merchant) { n.Edges.Merchant = e }); err != nil {
			return nil, err
		}
	}
...
func (dq *DepartmentQuery) loadMerchant(ctx context.Context, query *MerchantQuery, nodes []*Department, init func(*Department), assign func(*Department, *Merchant)) error {
...
func (dq *DepartmentQuery) loadStore(ctx context.Context, query *StoreQuery, nodes []*Department, init func(*Department), assign func(*Department, *Store)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Department)
	for i := range nodes {
		fk := nodes[i].StoreID
...
func (dq *DepartmentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := dq.ctx.Fields; len(fields) > 0 {
		...
		if dq.withMerchant != nil {
			_spec.Node.AddColumnOnce(department.FieldMerchantID)
		}
		if dq.withStore != nil {
			_spec.Node.AddColumnOnce(department.FieldStoreID)
		}
	}
