...
func (mq *MenuQuery) QueryStores() *StoreQuery {
...
func (mq *MenuQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (mq *MenuQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (mq *MenuQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (mq *MenuQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (mq *MenuQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (mq *MenuQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (mq *MenuQuery) WithStores(opts ...func(*StoreQuery)) *MenuQuery {
	query := (&StoreClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MenuQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Menu, error) {
	...
	if query := mq.withStores; query != nil {
		if err := mq.loadStores(ctx, query, nodes,
			func(n *Menu) { n.Edges.Stores = []*Store{} },
			func(n *Menu, e *Store) { n.Edges.Stores = append(n.Edges.Stores, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MenuQuery) loadItems(ctx context.Context, query *MenuItemQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *MenuItem)) error {
	...
	nodeids := make(map[uuid.UUID]*Menu)
	for i := range nodes {
...
func (mq *MenuQuery) loadStores(ctx context.Context, query *StoreQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *Store)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Menu)
	nids := make(map[uuid.UUID]map[*Menu]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(menu.StoresTable)
		s.Join(joinT).On(s.C(store.FieldID), joinT.C(menu.StoresPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(menu.StoresPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(menu.StoresPrimaryKey[0]))
...
func (mq *MenuQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(menu.Table, menu.Columns, sqlgraph.NewFieldSpec(menu.FieldID, field.TypeUUID))
	_spec.From = mq.sql
