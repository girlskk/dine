...
const (
	DineCode = "CateringScore"
)

var _ PaymentProvider = (*ZxhPaymentProvider)(nil)

type ZxhPaymentProvider struct {
	mgr         *zxh.Manager
	credentials *zxh.Credentials
}

func NewZxhPaymentProvider(mgr *zxh.Manager, mchID, accessKey string) *ZxhPaymentProvider {
	return &ZxhPaymentProvider{
		mgr:         mgr,
		credentials: zxh.NewCredentials(mchID, accessKey),
	}
}

func (p *ZxhPaymentProvider) Provider() domain.PayProvider {
	return domain.PayProviderZhiXinHua
}

func (p *ZxhPaymentProvider) MchID() string {
	return p.credentials.MchID
}

func (p *ZxhPaymentProvider) Payment(ctx context.Context, seqNo string, params *PaymentParams) (res *PaymentResult, err error) {
	span, ctx := opentracing.StartSpanFromContext(ctx, "ZxhPaymentProvider.Payment")
	defer func() {
		util.SpanErrFinish(span, err)
	}()

	logger := logging.FromContext(ctx).Named("ZxhPaymentProvider.Payment")
	...
	return &PaymentResult{
		Req:     reqRaw,
		Resp:    respRaw,
		Channel: domain.PayChannelPoint,
	}, nil
}

func transPointPayRespErr(err error, preMsg string) error {
	if err == nil {
		return nil
	}

	if zxh.IsAPIError(err) {
		return domain.ParamsErrorf("%s: %s", preMsg, err)
	}

	return fmt.Errorf("%s: %w", preMsg, err)
}

var _ PaymentCallbackProvider = (*ZxhPaymentCallbackProvider)(nil)

type ZxhPaymentCallbackProvider struct{}

func NewZxhPaymentCallbackProvider() *ZxhPaymentCallbackProvider {
	return &ZxhPaymentCallbackProvider{}
}

func (p *ZxhPaymentCallbackProvider) Callback(ctx context.Context, callback *domain.PaymentCallback, payment *domain.Payment) (res *PaymentCallbackResult, err error) {
