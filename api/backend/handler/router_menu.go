...
func (h *RouterMenuHandler) Create() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		params := &domain.CreateRouterMenuParams{
			UserType:  domain.UserTypeBackend,
...
//	@Param			id	path	string	true	"菜单ID"
//	@Success		200	"No Content"
//	@Router			/common/router_menu/{id}/enabledd [put]
func (h *RouterMenuHandler) Enabledd() gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx := c.Request.Context()
		logger := logging.FromContext(ctx).Named("RouterMenuHandler.Enabledd")
		ctx = logging.NewContext(ctx, logger)
		c.Request = c.Request.Clone(ctx)
		...
		if err := h.Interactor.UpdateRouterMenu(ctx, params); err != nil {
			...
			err = fmt.Errorf("failed to enabledd router menu: %w", err)
			c.Error(err)
			return
		}
