...
func NewAuth(handlers []ugin.Handler, userInteractor domain.BackendUserInteractor) *Auth {
	var prefixes []string
	for _, h := range handlers {
		switch v := h.(type) {
		case interface{ NoAuths() []string }:
			for _, n := range v.NoAuths() {
				prefixes = append(prefixes, backend.ApiPrefixV1+n)
			}
		}
	}
...
func (u *Auth) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		ctx = domain.NewBackendUserContext(ctx, u)
