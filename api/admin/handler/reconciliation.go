...
func (h *ReconciliationHandler) Routes(r gin.IRouter) {
	...
	r.POST("/generate-daily-records-by-date", h.GenerateDailyRecordsByDate())
}
...
func (h *ReconciliationHandler) ListDetails() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		res, err := h.ReconciliationRecordInteractor.ListDetails(ctx, req.ID, 0)
		if err != nil {
			if domain.IsParamsError(err) {
				c.Error(uerr.BadRequest(err.Error()))
			} else {
...
func (h *ReconciliationHandler) ExportList() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		submitParams, err := domain.BuildDataExportSubmitParams(0, domain.DataExportTypeReconciliationRecordListExport, params, fileName, user)
		if err != nil {
...
func (h *ReconciliationHandler) ExportListDetails() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		submitParams, err := domain.BuildDataExportSubmitParams(0, domain.DataExportTypeReconciliationRecordDetailsExport, params, fileName, user)
		if err != nil {
			c.Error(fmt.Errorf("failed to build data export submit params: %w", err))
			return
		}
...
//	@Tags		财务对账
//	@Security	BearerAuth
//	@Summary	生成指定日期的财务对账单
//	@Param		data	body	types.ReconciliationGenerateDailyRecordsByDateReq	true	"请求参数"
//	@Success	200		"No Content"
//	@Router		/reconciliation/generate-daily-records-by-date [post]
func (h *ReconciliationHandler) GenerateDailyRecordsByDate() gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx := c.Request.Context()
		span, ctx := opentracing.StartSpanFromContext(ctx, "ReconciliationHandler.GenerateDailyRecordsByDate")
		defer span.Finish()
		logger := logging.FromContext(ctx).Named("ReconciliationHandler.GenerateDailyRecordsByDate")
		ctx = logging.NewContext(ctx, logger)
		c.Request = c.Request.Clone(ctx)

		var req types.ReconciliationGenerateDailyRecordsByDateReq
		if err := c.ShouldBind(&req); err != nil {
			c.Error(uerr.BadRequest(err.Error()))
			return
		}

		if !req.RecordDate.IsValid() {
			c.Error(uerr.BadRequest("账单日期不能为空"))
			return
		}

		err := h.ReconciliationRecordInteractor.GenerateDailyRecordsByDate(ctx, req.RecordDate.ToTime())
		if err != nil {
			c.Error(err)
			return
		}
