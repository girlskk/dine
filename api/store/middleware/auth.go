...
func NewAuth(handlers []ugin.Handler, userInteractor domain.StoreUserInteractor) *Auth {
	var prefixes []string
	for _, h := range handlers {
		switch v := h.(type) {
		case interface{ NoAuths() []string }:
			for _, n := range v.NoAuths() {
				prefixes = append(prefixes, store.ApiPrefixV1+n)
			}
		}
	}
...
func (u *Auth) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		ctx = domain.NewStoreUserContext(ctx, u)
